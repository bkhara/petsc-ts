...
PetscErrorCode InitialState(DM da, Vec Y, double noiselevel, AppCtx* user) {
    ...
    int i, j;
    double sx, sy;
    const double ledge = (user->L - 0.5) / 2.0, redge = user->L - ledge;
    // declare some analytical function on top the random variables between x = 1.0, to x = 1.5
    DMDACoor2d** aC;
    Field** aY; // 2D array

    ierr = VecSet(Y, 0.0);
    CHKERRQ(ierr);
    ierr = VecSetRandom(Y,NULL);
    CHKERRQ(ierr);
    ierr = VecScale(Y, noiselevel);
    CHKERRQ(ierr);

    ierr = DMDAGetLocalInfo(da, &info);
    CHKERRQ(ierr);
    ierr = DMDAGetCoordinateArray(da, &aC);
    CHKERRQ(ierr); // ac[j][i].x or ac[j][i].y
    ierr = DMDAVecGetArray(da, Y, &aY);
    CHKERRQ(ierr);
    ...
    ierr = DMDAVecRestoreArray(da, Y, &aY);
    CHKERRQ(ierr);
    ierr = DMDARestoreCoordinateArray(da, &aC);
    CHKERRQ(ierr);
    return 0;
}
...
PetscErrorCode FormRHSJacobianLocal(DMDALocalInfo* info, double t, Field** aY, Mat J, Mat P, AppCtx* user) {
    ...
    for (j = info->ys; j < info->ys + info->ym; j++) {
        ...
        for (i = info->xs; i < info->xs + info->xm; i++) {
            ...
            v[0] = -v2 - user->phi;
            v[1] = -2.0 * uv;
            ierr = MatSetValuesStencil(P, 1, &row, 2, col, v, INSERT_VALUES);
            CHKERRQ(ierr);
    ...
    for (j = info->ys; j < info->ys + info->ym; j++) {
        ...
        for (i = info->xs; i < info->xs + info->xm; i++) {
            ...
            for (c = 0; c < 2; c++) {
                ...
                col[8].j = j + 1;
                val[8] = -CC;
                ierr = MatSetValuesStencil(P, 1, &row, 9, col, val, INSERT_VALUES);
                CHKERRQ(ierr);
            }
        }
    }
...
int main(int argc, char** argv) {
    ...
    ierr = DMDACreate2d(PETSC_COMM_WORLD, DM_BOUNDARY_PERIODIC, DM_BOUNDARY_PERIODIC, DMDA_STENCIL_BOX, 3, 3,
                        PETSC_DECIDE,PETSC_DECIDE, 2, 1, NULL,NULL, &da);
    CHKERRQ(ierr);
    ierr = DMSetFromOptions(da);
    CHKERRQ(ierr);
    ierr = DMSetUp(da);
    CHKERRQ(ierr);
    ierr = DMDASetFieldName(da, 0, "u");
    CHKERRQ(ierr);
    ierr = DMDASetFieldName(da, 1, "v");
    CHKERRQ(ierr);
    ierr = DMDAGetLocalInfo(da, &info);
    CHKERRQ(ierr);

    ierr = DMDASetUniformCoordinates(da, 0.0, user.L, 0.0, user.L, -1.0, -1.0);
    CHKERRQ(ierr);

    ierr = TSCreate(PETSC_COMM_WORLD, &ts);
    CHKERRQ(ierr);
    ierr = TSSetDM(ts, da);
    CHKERRQ(ierr); // Link the time-stepper with the DMDA
    ...
    CHKERRQ(ierr);
    ierr = DMDATSSetRHSJacobianLocal(da, (DMDATSRHSJacobianLocal)FormRHSJacobianLocal, &user);
    CHKERRQ(ierr);
    ierr = DMDATSSetIFunctionLocal(da, INSERT_VALUES, (DMDATSIFunctionLocal)FormIFunctionLocal, &user);
    CHKERRQ(ierr);
    ierr = DMDATSSetIJacobianLocal(da, (DMDATSIJacobianLocal)FormIJacobianLocal, &user);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = TSSetMaxTime(ts, 15000.0);
    CHKERRQ(ierr);
    ierr = TSSetTimeStep(ts, 5.0);
    CHKERRQ(ierr);
    ...
    CHKERRQ(ierr);
    ierr = InitialState(da, x, noiselevel, &user);
    CHKERRQ(ierr);
    ierr = TSSolve(ts, x);
    CHKERRQ(ierr);
